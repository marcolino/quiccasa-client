...
export default function TermsOfUse() {
  ...
  return (
    <div className={classes.termsofuse}>
      ...
      <p>Accedendo a questo sito, Voi accettate i termini e le condizioni seguenti. Vi chiediamo di non continuare ad usare {config.appName} nel caso in cui non fosse d'accordo con tutti i termini e le condizioni in questa pagina.</p>
      <h3>Cookie</h3>
      <p>Il sito utilizza cookie per personalizzare l'esperienza online. Accedendo a {config.appName}, accetta l'uso dei cookie richiesti.</p>
      <p>Un cookie è un file di testo che viene posizionato sull'hard disk da un server web. I cookie non possono essere usati per eseguire programmi o inviare virus al computer. I cookie vengono assegnati unicamente a Voi e possono essere letti solo dal server web nel dominio che ha emesso il cookie.</p>
      <p>Potremmo usare i cookie per raccogliere, archiviare e tracciare informazioni per finalità statistiche e di marketing al fine di gestire il nostro sito web. Vi consentiamo di accettare o rifiutare i cookie opzionali. Sono presenti dei cookie necessari indispensabili per il funzionamento del sito. Tali cookie non richiedono il consenso dato che sono attivi in ogni caso. Le chiediamo di tenere a mente che l'accettazione dei cookie richiesti prevede anche l'accettazione dei cookie di terzi, che potrebbero provenire da servizi forniti da terzi nel caso in cui decida di usare tali servizi sul nostro sito. Ad esempio, una schermata video fornita da terzi e integrata sul nostro sito.</p>
      <h3>Licenza</h3>
      <p>A meno che indicato altrimenti, {config.companyHomeSiteName} e/o gli affiliati con licenza detengono i diritti di proprietà intellettuale per tutto il materiale su {config.appName}. Tutti i diritti di proprietà intellettuale sono riservati. Potete accedervi tramite {config.appName} per uso personale soggetto a restrizioni indicate nei termini e nelle condizioni seguenti.</p>
      <p><strong>Non è consentito:</strong></p>
      <ul>
        <li>Copiare o ripubblicare il materiale di {config.appName}</li>
      ...
      <p>Nel caso in cui Voi rappresentiate una delle organizzazioni indicate nel paragrafo 2 sopra e siate interessati ad aggiungere un collegamento al nostro sito, dovete informarci inviando un'email a {config.companyHomeSiteName}. Vi chiediamo di includere nome, denominazione dell'organizzazione, informazioni di contatto e URL del sito, una lista di URL su cui intende aggiungere un collegamento al nostro sito e una lista di URL sul nostro sito che intende collegare. Vi chiediamo di attendere 2-3 settimane per una risposta.</p>
